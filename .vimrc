...
"-- compile while c programs have saved
autocmd BufWritePost,FileWritePost *.c :setlocal makeprg=g++\ -Wall\ -o\ %:r.exe\ %
autocmd BufWritePost,FileWritePost *.c :make
autocmd BufWritePost,FileWritePost *.c :cw

"run C code if make successfully
autocmd QuickfixCmdPost make call Aftercw()
function! Aftercw()
  "no any error after make"
  if len(getqflist()) == 0
    !echo -e "----[Built Successfully, Run Program ...]----\n" && ./%:r.exe 
  endif
endfunction
